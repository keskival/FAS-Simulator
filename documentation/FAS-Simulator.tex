...
Currently FMS processes are often modelled as colored Petri nets \cite{saitou2002robust}.
...
with log messages labelled with the process instance. If the process instances are known for log messages, the corresponding Petri Net model is considered ``colored''.
In practice, process instances can only be known in the process
...
in fact might affect multiple process instances at once. Flexible Manufacturing Systems are reconfigured often for different workloads and manually reconfiguring the anomaly
...
variants, such as $\alpha^{++}$ \cite{wen2007mining} can discover some simple cases of controlled choice.

Deinterleaving the logs would simplify the automatas required for evaluating the allowed sequences. For example, if the log events contained the identifier of the product
...
TODO: Run $\alpha^{++}$, genetic process mining against the log.
